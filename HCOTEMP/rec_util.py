...
def new_test_epoch(model, loader, args):
    hr_5, hr_10, hr_20 = [], [], []
    model.eval()
    T = torch.tensor([0]).to(args.device)
    for batch_idx, batch in enumerate(loader):
        b_user_records, b_item_records, b_uids, b_iids, b_ulens, b_ilens, _ = batch
        b_user_records = b_user_records.to(args.device)
        b_item_records = b_item_records.to(args.device)
        b_uids = b_uids.to(args.device)
        b_iids = b_iids.to(args.device)
        ...
        # b_labels = b_labels.to(args.device)
        outputs = model(b_user_records, b_item_records, b_uids, b_iids, b_ulens, b_ilens)
        p = torch.argsort(outputs.reshape(-1,).unsqueeze(0), dim=1, descending=True)
        hr_5.append(HR(T, p, 5))
        hr_10.append(HR(T, p, 10))
        hr_20.append(HR(T, p, 20))

    return round(sum(hr_5) / len(hr_5), 6), round(sum(hr_10) / len(hr_10), 6), round(sum(hr_20) / len(hr_20), 6)
