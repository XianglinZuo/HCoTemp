...
from rec_util import new_train_epoch, new_valid_epoch, load_pkl, AMDataset, my_fn, new_test_epoch
...
def parse_args():
    ...
    train_settings.add_argument('--batch_eval', type=int, default=100,
                                help='dev batch size')
...
def func_test(args, file_paths, gpu, ngpus_per_node):
    ...
    # test_loss = new_test_epoch(model, test_loader, args)
    hr_5, hr_10, hr_20 = new_test_epoch(model, test_loader, args)
    logger.info('Test HR@5 - {}, HR@10 - {}, HR@20 - {}'.format(hr_5, hr_10, hr_20))
...
if __name__ == '__main__':
    ...
    class FilePaths(object):
        def __init__(self):
            self.train_file = os.path.join(args.processed_dir,
                                           'train_{}.pkl'.format(args.P) if args.period else 'train.pkl')
            self.valid_file = os.path.join(args.processed_dir,
                                           'valid_{}.pkl'.format(args.P) if args.period else 'valid.pkl')
            self.test_file = os.path.join(args.processed_dir,
                                          'test_{}.pkl'.format(args.P) if args.period else 'test.pkl')
    ...
    if args.train:
        ...
        # func_train(args, file_paths, args.gpu if args.local_rank == -1 else args.local_rank, 1 if args.gpu != 2 else 2)
    if args.test:
        func_test(args, file_paths, args.local_rank, 2)
        # func_test(args, file_paths, args.gpu if args.local_rank == -1 else args.local_rank, 1 if args.gpu != 2 else 2)
